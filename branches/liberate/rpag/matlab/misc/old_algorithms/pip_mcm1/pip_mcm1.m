...
function [op_cnt,S,comp_time] = pip_mcm1(coeff,mode)
...
for s=s_max:-1:2
  ...
  while length(C)~=0
    ...
    %disp(['x=',num2str(x)]);
    if isempty(P) 
      %if P=[], find the best pair x,y to realize at most target values:
      [x_best,y_best] = find_best_predecessor_pair(C,P,As,c_max);
    else
      %if P~=[], find x to realize at most target values in combination with P:
      [x_best] = find_best_single_predecessor(C,P,As,c_max);
      y_best=0;
      if x_best < 0
        %if no single predecessor was found, search for best pair:
        [x_best,y_best] = find_best_predecessor_pair(C,P,As,c_max);
      end
    end
