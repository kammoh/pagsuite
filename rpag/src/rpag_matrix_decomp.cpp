...
namespace PAGSuite
{
  ...
  void rpag_matrix_decomp::split_target_into_2(set<vec_t> target_split_set, vector<bool> partion_helper, vector<pair<set<vec_t>, vector<bool> > > &result)
  {
      ...
      new_split_set_1.second.clear();
      new_split_set_1.first.clear();
      new_split_set_1.second.resize(partion_helper.size());
      new_split_set_2.second.clear();
      new_split_set_2.first.clear();
      new_split_set_2.second.resize(partion_helper.size());
      ...
      {
          new_target_1.clear();
          new_target_1.resize(partion_helper.size());
          new_target_2.clear();
          new_target_2.resize(partion_helper.size());
          ...
          if (new_target_1 != 0)
          { new_split_set_1.first.insert(new_target_1); }
          if (new_target_2 != 0)
          { new_split_set_2.first.insert(new_target_2); }

      }

      // add the new generatet Problems to the result set if it is not empty
      if (new_split_set_1.first.size() != 0)
      { result.push_back(new_split_set_1); }
      if (new_split_set_2.first.size() != 0)
      { result.push_back(new_split_set_2); }
  }
  ...
  void rpag_matrix_decomp::split_target_into_3(set<vec_t> target_split_set, vector<bool> partion_helper, vector<pair<set<vec_t>, vector<bool> > > &result)
  {
      ...
      new_split_set_2.first.clear();
      new_split_set_2.second.resize(partion_helper.size());
      new_split_set_3.second.clear();
      new_split_set_3.first.clear();
      new_split_set_3.second.resize(partion_helper.size());
      ...
      for (vec_t t : target_split_set)
      {
          ...
          new_target_2.resize(partion_helper.size());
          new_target_3.clear();
          new_target_3.resize(partion_helper.size());
          ...
          { new_split_set_2.first.insert(new_target_2); }
          if (new_target_3 != 0)
          { new_split_set_3.first.insert(new_target_3); }
      ...
      if (new_split_set_2.first.size() != 0)
      { result.push_back(new_split_set_2); }
      if (new_split_set_3.first.size() != 0)
      { result.push_back(new_split_set_3); }
  }
  ...
  void rpag_matrix_decomp::perform_decomposition(set<vec_t> *target_fun_set)
  {
      ...
      for (int i = 0; i < partition_helper.size(); ++i)
      { partition_helper[i] = true; } //in the beginning all alements in all targets belong to the Problem
