...
namespace PAGSuite
{
            ...
            if (den != 0)
            {
              ...
              if (num % den == 0)
              {
                ...
                if (nonzeros(p2) <= nz_max)
                {
                  ...
                  {
                    IF_VERBOSE(4) cout << "(p1,p2)=[" << p1 << "," << p2 << "]" << ", with s12=" << s12 << ", s22=" << s22 << ", l11=" << l11 << ", l12=" << l12 << ", l21=" << l21 << ", l22=" << l22 << ", r1=" << r1 << ", r2=" << r2
                                       << endl;
  ...
  public:
    ...
    void permutation_for_predeccessor_pairs(vector<int_t> &nonzero_values, vector<int> &nonzero_shift_factor, vector<int> &nonzero_element, set<triplet<T, T, T> > *predecessor_triplet_set, int &ws_max, int nz_max, bool rekursion, vec_t p3,
...
//  void set_cost_model(cost_model_t cost_model, rpag_pointer *rpagp)
  ...
  int rpag_base<T>::optimize()
  {
    ...
    int first_decision_predecessor_stage = -1;
    int first_decision_predecessor_stage_best = -1;
    set<T> first_decison_predecessor;
    set<T> first_decison_predecessor_best;
          ...
          if ((best_single_predecessor != -1) && (best_single_predecessor != 0))
          {
            ...
            if (first_decision_done == false)
            {
              ...
              *first_decision_predecessor_stage = (s - 1);
              ...
              if (first_decision_break)
              { return 1; } //just to get the next predecessor in a already computet case
              //TODO REmove this line
