    ...
    */
    ...
    for (int k = 0; k <= k_max; k++)
    {
      for (int l = 0; l <= k; l++)
      {
        ...
        { permut_border = 6; }//if k = l it's not necessary to switch k and l
    ...
    for (int k = 0; k <= k_max; k++)
    {
      for (int l = 0; l <= k; l++)
      {
        ...
        for (int permut_counter = 0; permut_counter < permut_border; permut_counter++)
        {
          ...
          s = norm((r1 << a) + (r2 << b) + (r3 << c), &sign_switched);
          //((!ternary_sign_filter) || (!sign_switched)) is to filter sign cominations with more then 1 negative sign.
          if ((s != 0) && (abs(s) <= c_max) && ((!ternary_sign_filter) || just_one_negative_sign((false ^ r1_sign ^ sign_switched), (false ^ r2_sign ^ sign_switched), (false ^ r3_sign ^ sign_switched))))
            (*successor_set).insert(fundamental(s));
#ifdef HEAVY_DEBUG_OUTPUT
          IF_VERBOSE(8) cout << s << "=" << "2^" << a << " * " << r1 << " + " << "2^" << b << " * " << r2 << " + " << "2^" << c << " * " << r3 << std::endl;
#endif
          ...
          s = norm((r1 << a) + (r2 << b) - (r3 << c), &sign_switched);
          //((!ternary_sign_filter) || (!sign_switched)) is to filter sign cominations with more then 1 negative sign.
          if ((s != 0) && (abs(s) <= c_max) && ((!ternary_sign_filter) || just_one_negative_sign((false ^ r1_sign ^ sign_switched), (false ^ r2_sign ^ sign_switched), (true ^ r3_sign ^ sign_switched))))
            (*successor_set).insert(fundamental(s));
#ifdef HEAVY_DEBUG_OUTPUT
          IF_VERBOSE(8) cout << s << "=" << "2^" << a << " * " << r1 << " + " << "2^" << b << " * " << r2 << " - " << "2^" << c << " * " << r3 << std::endl;
#endif
          ...
          s = norm((r1 << a) - (r2 << b) + (r3 << c), &sign_switched);
          //((!ternary_sign_filter) || (!sign_switched)) is to filter sign cominations with more then 1 negative sign.
          if ((s != 0) && (abs(s) <= c_max) && ((!ternary_sign_filter) || just_one_negative_sign((false ^ r1_sign ^ sign_switched), (true ^ r2_sign ^ sign_switched), (false ^ r3_sign ^ sign_switched))))
            (*successor_set).insert(fundamental(s));
#ifdef HEAVY_DEBUG_OUTPUT
          IF_VERBOSE(8) cout << s << "=" << "2^" << a << " * " << r1 << " - " << "2^" << b << " * " << r2 << " + " << "2^" << c << " * " << r3 << std::endl;
#endif
          ...
          s = norm((r1 << a) - (r2 << b) - (r3 << c), &sign_switched);
          //((!ternary_sign_filter) || (!sign_switched)) is to filter sign cominations with more then 1 negative sign.
          if ((s != 0) && (abs(s) <= c_max) && ((!ternary_sign_filter) || just_one_negative_sign((false ^ r1_sign ^ sign_switched), (true ^ r2_sign ^ sign_switched), (true ^ r3_sign ^ sign_switched))))
            (*successor_set).insert(fundamental(s));
#ifdef HEAVY_DEBUG_OUTPUT
          IF_VERBOSE(8) cout << s << "=" << "2^" << a << " * " << r1 << " - " << "2^" << b << " * " << r2 << " - " << "2^" << c << " * " << r3 << std::endl << std::endl;
#endif
