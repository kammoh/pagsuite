...
namespace PAGSuite
{
  ...
  string rpag_info(vector<string> &start_arguments, bool new_line)
  {
    ...
    output_stream << "  last full compile \t\t = " << __DATE__ << " " << __TIME__;
    if (new_line == true)
    { output_stream << std::endl << std::endl; }
    unsigned long long MAX_INT32_var = MAX_INT32;
    unsigned long long MAX_UINT32_var = MAX_UINT32;

    output_stream << "  #" << "MAX_INT32 \t\t\t = " << MAX_INT32_var;
    if (new_line == true)
    { output_stream << std::endl; }
    output_stream << "  #" << "MAX_UINT32 \t\t\t = " << MAX_UINT32_var;
    if (new_line == true)
    { output_stream << std::endl; }
    output_stream << "  #" << "USE_NEW_NZ_COMPARE_FUNTION \t = " << USE_NEW_NZ_COMPARE_FUNTION;
    if (new_line == true)
    { output_stream << std::endl; }
    output_stream << "  #" << "USE_TOPOLOGY_E \t\t = " << USE_TOPOLOGY_E;
    if (new_line == true)
    { output_stream << std::endl; }
    output_stream << "  #" << "COST_FF_DEFAULT_ASIC \t = " << COST_FF_DEFAULT_ASIC;
    if (new_line == true)
    { output_stream << std::endl; }
    output_stream << "  #" << "COST_FA_DEFAULT_ASIC \t = " << COST_FA_DEFAULT_ASIC;
    if (new_line == true)
    { output_stream << std::endl; }
    output_stream << "  #" << "COST_FF_DEFAULT_FPGA \t = " << COST_FF_DEFAULT_FPGA;
    if (new_line == true)
    { output_stream << std::endl; }
    output_stream << "  #" << "COST_FA_DEFAULT_FPGA \t = " << COST_FA_DEFAULT_FPGA;
    if (new_line == true)
    { output_stream << std::endl; }
