...
namespace PAGSuite
{
  ...
  string realization_row<T>::operator[](unsigned int i)
  {
    ...
    switch (i)
    {
      ...
      case 10:
        ...
        break;
      default:
        std::cout << "realization_row operator[] undefined argument" << std::endl;
        exit(-1);
    }
    return s.str();
  }
  ...
  string realization_row<T>::to_ss_standard()
  {
    ...
    else if (type() == 'A')
    {
      ...
      if (C != 0)
      { s << ",[" << C << "]," << stageC << "," << eC; }//for ternary adders
      s << "}";
    }
    else
    {
...
// nessesary for getExponents(3 adder) to support vec_t and int_t
  inline bool is_it_negative(int_t Z)
  { return (Z < 0); }
  ...
  inline bool is_it_negative(int Z)
  { return (Z < 0); }// nessesary for sign_filter option
  inline bool is_it_negative(bool Z)
  { return Z; }// nessesary for sign_filter option // false = positive sign; true= negative sign
  ...
  bool getExponents(T &a, T &b, T &c, T &w, int *eA, int *eB, int *eC, int *signA, int *signB, int *signC, int_t c_max, bool ternary_sign_filter)
  {
    ...
    for (int r_shift = 0; r_shift <= k_max; ++r_shift)
    {
      for (permut = 0; permut <= 2; ++permut)// right shifts can only be detected between a and b. If ther is one between c and another thy have to be switched.
      {
        ...
        {
          case 0:
            break;//initial set. test for right shift beetween the original a and b.
          case 1:
            swap(a, c);
            swap(*eA, *eC);
            swap(*signA, *signC);
            break; // test for right shift beetween the original c and b.
          case 2:
            swap(b, c);
            swap(*eB, *eC);
            swap(*signB, *signC);
            break; // test for right shift beetween the original c and a.
          default:
            std::cout << "error in getExponents permut is not 0,1 or 2" << endl;
            exit(-1);
        }
          ...
          for (int sign_loop = 0; sign_loop <= 1; ++sign_loop)
          {
            switch (sign_loop)
            {
              ...
              case 1:
                ...
                break;
              default:
                std::cout << "error in getExponents sign_loop is not 0 or 1" << endl;
                exit(-1);
            }
            ...
            if (ternary_sign_filter)
            {
              ...
              if (just_one_negative_sign((*signA), (*signB), (*signC)) == false)
              { found = false; }
            }
            else
            {
            ...
            if (found == true)
            {
              ...
              switch (permut)
              {
                case 2:
                  swap(b, c);
                  swap(*eB, *eC);
                  swap(*signB, *signC); // switch b&c and a&c to create the original set
                case 1:
                  swap(a, c);
                  swap(*eA, *eC);
                  swap(*signA, *signC); // switch         a&c to create the original set
                case 0:
                  break;                                         // initial set.                     no switching
                default:
                  std::cout << "error in getExponents permut is not 0,1 or 2" << endl;
                  exit(-1);
              }
            ...
            if (ternary_adders && (!exponentsFound))
            {
              ...
              for (predecessor_set_iter1 = (pipeline_set)[s - 1].begin(); predecessor_set_iter1 != (pipeline_set)[s - 1].end(); ++predecessor_set_iter1)
              {
                ...
                for (predecessor_set_iter2 = predecessor_set_iter1; predecessor_set_iter2 != (pipeline_set)[s - 1].end(); ++predecessor_set_iter2)
                {
                  ...
                  {
                    if (skip_predecessor(w, (*predecessor_set_iter3), dont_skip))
                    { continue; }
                    a = *predecessor_set_iter1;
                    b = *predecessor_set_iter2;
                    c = *predecessor_set_iter3;
                    exponentsFound = getExponents(a, b, c, w, &eA, &eB, &eC, &signA, &signB, &signC, c_max, ternarry_sign_filter);
                    if (ternarry_sign_filter && !just_one_negative_sign(signA, signB, signC))
                    { exponentsFound = false; }//do not use ternarry adders with more than one negative sign;
                    if (exponentsFound) break;
                  }
                  if (exponentsFound) break;
                }
